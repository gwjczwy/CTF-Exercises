...
logPath=r'C:\Users\arnoux\Desktop\训练平台\sql\log.txt'
#########################
#主页
@login_required
def index(requests):
...
# 获取日志
def getLog(requests):
    with open(r'sql\log.txt','rt') as f:
        contnet = "</br>".join(f.readlines()[-10:])
    req={'message':'success','content':contnet}
    return HttpResponse(dumps(req),content_type="application/json")
...
# /flag/flag.txt
def getflag(requests):
    return HttpResponse(''.join(open(r'sql\log.txt','rt').readlines()))
